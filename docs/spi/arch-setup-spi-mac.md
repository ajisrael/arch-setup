# Arch Linux Manual Installation Command Summary (SPI Keyboard Fix Variant)

Variant of `docs/arch-setup-mac.md` for the MacBookPro12,1 that additionally
...
`docs/spi/macbookpro12-1-keyboard-spi-fix.md` (and
`docs/macbookpro12-1-keyboard-issue.md` for the full investigation).
...
> Gotcha: `sudo echo "..." > /proc/acpi/call` does NOT work - the shell
> redirect is opened by your unprivileged user, so you get
> `Permission denied`. Use `su -` or wrap the whole command:
> `sudo sh -c 'echo "\\_SB.PCI0.SPI1.SPIT.UIST" > /proc/acpi/call'`.
...
```bash
cat ~/.ssh/id_ed25519.pub | ssh <user>@<arch-ip> 'install -d -m700 ~/.ssh && cat >> ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys'
```
